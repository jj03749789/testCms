...
import "iristest2/model"

//管理员服务 标准的开发模式将每个实体的提供的功能以接口标准的形式定义，供控制层进行调用

type AdminService interface{
	//通过管理员用户名+密码 获取管理员实体 如果查询到，返回管理层实体，并返回true
	//否则 返回nil ,false
...
//通过用户名和密码查询管理员
func(ac *adminService) GetByAdminNameAndPassword()(model.Admin,bool){
	var admin model.Admin
	ac.engine.Where("admin_name=? amd pwd=?",username,password).Get(&admin)
